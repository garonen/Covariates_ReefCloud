...
writeRaster(bathy_pal,
            'F:/ReefCloud/Covariates_ReefCloud/Outputs/Bathymetry/Bathymetry_Palau.tif',
             overwrite = T,
             progress = 'text',
             format = 'GTiff',
             datatype = 'FLT4S',
...
#print(colnames(mod_year) <- as.character(i))
# can instead use some variation of maxValue()
# hist(m_wks)
# mean_wks <- as.integer(mean(m_wks))
#modal(m_wks)
# ci_wks <- as.integer()
...
# year <- as.integer(year(getZ(ssta_dhw_pal)))
# ts_year <- subset(ssta_dhw_pal, (grep(year, names(ssta_dhw_pal))))
# ts_year[is.na(ts_year)] <- -9999
# m_ord2 <- calc(ts_year, function (x) {which.max(x)})
#   which.max(m_ord2)
#   
# }
# summary(ts_year)
...
#     
#         
#     m_ord1 <- calc(ts_year, fun = function(x, na.rm) x[order(x, decreasing = T)][1:4])
#     which.max(m_ord1)
#     
# #    
# 
...
# 
#   myyear <- subset(ssta_dhw_pal, which(year(getZ(ssta_dhw_pal)) == i))
#  print(myyear)
#   
# zz <- range(cellStats(myyear,range))
# m_ord <- calc(myyear, fun = function(x, na.rm) x[order(x, decreasing = T)][1:4])
# #print(m_ssta1 <- calc(myyear, fun = max, na.rm = T))
# print(m_ord)
# which(myyear) == m_ord
# 
# }
...
# #  
# plot(m_ssta1)
# m_ssta1 <- calc(ssta_dhw_pal, fun = max, na.rm = T)
# maxBands = unique(m_ssta1)
# outRas = ssta_dhw_pal[[1]]
# for (i in 1:length(maxBands)) {
#   myBand = maxBands[i]
#   outRas[which(ssta_dhw_pal) == myBand] <- ssta_dhw_pal[[myBand]][ssta_dhw_pal == myBand]
# }
# 
# 
# m_ssta <- raster::calc(ssta_dhw_pal, function(x,...) 
#                                      max(x, unique(year(getZ(ssta_dhw_pal))),
#                                      na.rm = T)) 
# m_ssta
# 
# plot(m_ssta1) 
# 
# m_ssta <- zApply(ssta_dhw_pal,
#                  by = year(getZ(ssta_dhw_pal)),
#                  fun = 'max')
# values(m_ssta)
# values(ssta_dhw_pal[[1980]]) 
# m_ssta <- apply.yearly(ssta_dhw_rs, FUN = )
# findMethods(which.max)
# unique(year(getZ(ssta_dhw_pal)))
# endpoints(ssta_rs, 'years')
# which(max(ssta_dhw_pal))
